...
// Drop-in replacement for the original src/render/tileArt.ts. Keeps the SAME
// public API (drawTile / computeMeepleSpot / clearTileCache / prewarmTile /
// TILE_BUFFER_RES / DrawTileOpts) and the SAME segment + rotation contract, so
// engine / ai / meeples / board keep working unchanged. The difference is the
// art: every tile is now drawn at 48x48 in crisp, unsmoothed pixels with a
// warm/cool "Celeste" palette, hard dark-plum outlines and ordered dithering.
//
...
function renderBufData(def: TileDef, rot: number): Uint8ClampedArray {
  ...
  const roadCarve = roadSides.length ? dilate(buildRoadMask(roadSides), 1) : undefined;
  if (cities.length <= 1) {
